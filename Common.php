...
//Connection Settings @0-C52A43CD
...
//Initialize Common Variables @0-91BC27F1
...
//con_xls Connection Class @-B088075E
class clsDBcon_xls extends DB_Adapter
{
    function clsDBcon_xls()
    {
        $this->Initialize();
    }
    ...
    function Initialize()
    {
        ...
        $this->SetProvider($CCConnectionSettings["con_xls"]);
        parent::Initialize();
...
//End con_xls Connection Class

//ConnCarga Connection Class @-0C9160B7
class clsDBConnCarga extends DB_Adapter
{
    function clsDBConnCarga()
    {
        $this->Initialize();
    }
    ...
    {
        global $CCConnectionSettings;
        $this->SetProvider($CCConnectionSettings["ConnCarga"]);
        parent::Initialize();
        $this->DateLeftDelimiter = "'";
        $this->DateRightDelimiter = "'";
    }
...
//End ConnCarga Connection Class
...
//CCLoginUser @0-6D6BB91C
function CCLoginUser($login, $password)
{
    CCLogoutUser();
    $db = new clsDBConnCarga();
    $SQL = "SELECT id_usuario, nivel, password FROM [SO_usuario] WHERE usuario=" . $db->ToSQL($login, ccsText) . " AND password=" . $db->ToSQL($password, ccsText);
    $db->query($SQL);
